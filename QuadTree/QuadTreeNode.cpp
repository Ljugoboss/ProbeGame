...
{
	// Assign the new node Vec2fers while adding the occupants to the upper node
	for(std::unordered_set<QuadTreeOccupant*>::iterator it = occupants.begin(); it != occupants.end(); it++)
	{
		it._Ptr->_Myval->pQuadTreeNode = newNode;
...
Vec2f2i QuadTreeNode::GetPossibleOccupantPos(QuadTreeOccupant* pOc)
{
	Vec2f2i pos;

	// NOTE: The center position of the pOc->aabb is not tested, instead a corner is.
	// The center Vec2f is not required because if a corner is not in the
...
void QuadTreeNode::AddOccupant(QuadTreeOccupant* pOc)
{
	...
	if(hasChildren)
	{
		...
		Vec2f2i pos = GetPossibleOccupantPos(pOc);
	...
	else
	{
		...
		if(occupants.size() + 1 <= maximumOccupants || level > maxLevels)
		{
			...
			// Set the occupant's quad tree Vec2fer to this node
